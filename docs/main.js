        ...
        try {
          ...
          if (font && params.text) {
            ...
            if (allRaw.length > 0 && contoursBounds(allRaw).width > 1e-6) {
              ...
              params.glyphContours = glyphRaw.map((lineContours) => {
                if (lineContours.length === 0) return [];
                const lineBounds = contoursBounds(lineContours);
                const lineOffsetX = textAlign === "right"
                  ? rectWidth - padding - (lineBounds.minX + lineBounds.width) * scale
                  : textAlign === "center"
                    ? rectWidth / 2 - (lineBounds.minX + lineBounds.width / 2) * scale
                    : padding - lineBounds.minX * scale;
